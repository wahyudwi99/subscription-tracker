...
export default function AddSubscriptionPicker({
  userEmail,
  setSubsData,
  setAddOpen,
}: AddSubsDataType) {
  ...
  const insertNewSubs = async() => {
    const post_res = await fetch(`${process.env.NEXT_PUBLIC_ADMIN_BASE_URL}/add-subscription`, {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        subscription_period: `${duration} ${durationUnit}`,
  ...
  return (
    <div className="fixed z-50 inset-0 flex items-center justify-center">
      ...
      <div className="relative bg-white w-full max-w-md rounded-2xl shadow-xl p-6">
        <h2 className="text-lg font-semibold mb-1">Track a subscription</h2>
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            onClick={() =>
              useCustom ? setUseCustom(false) : setAddOpen(false)
            }
